"""Headless re-extraction of Day1 + Day2 with the fixed PlotExtractor engine
(nadir-tiered frame ranking, 5x5 GCP grid, affine+EPSG:7850 geotags, source
ICC embedded). Output: fresh dated dataset folder, margin 0 (bare plot)."""
import os
import sys
import threading
import time
...
ROOT = (r"F:\Ibrahim's Workspace2\Plant Emergency Dataset UWA"
        r"\2025_NUE+RootPhenotyping_I_DPIRD-20260902")
...
COMMON = dict(
    ...
    do_raw=True, raw_mode="native", engine="metashape", do_ortho=False,
...
if len(sys.argv) > 1:                       # e.g. "Day2" to re-run one day only
